...
unsigned char unitNames[][11] = {
        ...
        /* 10 | 16 */
                      {0xb3, 0xb1, 0xa0, 0xad, 0xb2, 0xaf, 0xae, 0xb1, 0xb3},         // Transport
        /* 11 | 17 */
                      {0xa1, 28,   0xa2, 0xae, 0xaf, 0xb3, 0xa4, 0xb1},                                 // Copter
        /* 12 | 18 */
...
void drawUI() {
    ...
    if (build_mode != 0) {
        ...
        if (m.board[c.x + m.left_view + m.boardWidth * (c.y + m.top_view)].base != NULL &&
            (unitPointer == NULL || c.selected != NULL)) {
        ...
        if (pA != NULL) {
            ...
            if (keyCode == 0x57) /* W */ {
                ...
                if (c.selected == NULL) {
                    ...
                    if (c.selected->takenAction) {
                        c.selected = NULL;
                        goto cannot_select_base;
                    }
                    if (c.selected == NULL) {
                        if (tempT->base != NULL && tempT->base->team == m.whoseTurn &&
                            tempT->base->type >= CAPTUREABLE_FACTORY) {
                    ...
                    if (unitLastX == 255) {
                        actually_move = 1;
                        if (c.selected->team == m.whoseTurn && !c.selected->takenAction &&
                            ...
                            if (attackCursor.selected != NULL) {
                                ...
                                if (c.selected->carrying != NULL && sizeofGetPossibleDrops(c.selected) != 0) {
                                    ...
                                    ++menuOptions.length;
                                } else if ((c.selected->index == UNIT_MECH || c.selected->index == UNIT_INFANTRY) &&
                                           m.board[c.selected->x + m.boardWidth * c.selected->y].base != NULL &&
                                           m.board[c.selected->x + m.boardWidth * c.selected->y].base->team !=
                                           c.selected->team) {
